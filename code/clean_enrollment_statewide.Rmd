...
subtitle: "CT EdSight Data - Statewide Enrollment"
...
```{r}
# Load data
df <- read_excel(here("data/raw/enrollment/CT_state_enrollment_data.xlsx"), sheet = "Results")
...
df <- df %>%
  ...
  rename(school_year = school_year,
         ...
         free_reduced_price_lunch_eligibility = free_reduced_price_lunch_eligibility)
```
...
# entity_type
df <- df %>%
  mutate(entity_type = factor("State"))
...
```{r}
df %>% saveRDS(here("data/clean/enrollment_statewide.rds"))
```
