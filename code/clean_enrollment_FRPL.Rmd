...
subtitle: "CT EdSight Data - Enrollment, FRPL Students"
...
```{r}
# Load data
df <- read_excel(here("data/raw/enrollment/CT_enrollment_data_FRPL_students_SCHOOL.xlsx"),
...
df <- df %>%
  ...
  rename(free_reduced_price_lunch_eligibility = student_group)
```
...
df <- df %>% 
  mutate(free_reduced_lunch_2_levels = case_when(free_reduced_price_lunch_eligibility == "Eligible for Free Meals" ~ "Eligible",
                       free_reduced_price_lunch_eligibility == "Eligible for Reduced-Price Meals" ~ "Eligible",
                       free_reduced_price_lunch_eligibility == "Not Eligible for Free or Reduced-Price Meals" ~ "Not Eligible",
                       free_reduced_price_lunch_eligibility == "All Students" ~ "All Students",
                       TRUE ~ NA),
...
```{r}
df %>% saveRDS(here("data/clean/enrollment/enrollment_FRPL.rds"))
```
