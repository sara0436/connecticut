...
subtitle: "CT EdSight Data - Enrollment, Race/Ethnicity"
...
```{r}
# Load data
df <- read_excel(here("data/raw/enrollment/CT_enrollment_data_race_202223_to_202324_SCHOOL.xlsx"),
...
df <- df %>%
  ...
  rename(race_ethnicity = student_group)
```
...
df %>%
  get_dupes(school_year, district_name, school_name, race_ethnicity)
```
...
```{r}
df %>% saveRDS(here("data/clean/enrollment/enrollment_race_ethnicity.rds"))
```
